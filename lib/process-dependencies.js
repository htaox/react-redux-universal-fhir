...
export class ProcessDependencies {
  ...
  exportDeps() {
    ...
    fs.writeFileSync(schemaSequenceFile, `export const schemaSequence = ${exportSequence};` ,{ encoding: 'utf8' });
    return this;
  }
  ...
  pyramidLoop(_deep){
    ...
    for (let k of processSequence){
      if (this.schemaProcessed[k] && this.schemaProcessed[k].filePath.default){
        //if the schema was skipped b/c boolean/string,etc we won't create a model for it
        this.processSequence.push(this.schemaProcessed[k].filePath.default);
      }
    }
