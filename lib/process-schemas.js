...
*/
  ...
  /**
   * sortProfileTypes Sorts profiles-types so Extension model goes first
   * @return {undefined}
   */
  sortProfileTypes(json) {

    let { entry } = json;

    //find first resource.id == 'Extension'
    const idx = entry.findIndex(x => x.resource.id == 'Extension');
  ...
  processProfiles() {
    ...
    for (let file of [profilesTypes, profilesResources, profilesOthers]) {
      ...
      //Sorts profiles-types so Extension model goes first
      json = this.sortProfileTypes(json);
