...
*/
    code = code.search(/^xhtml|uuid|uri|id|code|markdown|oid|string|time$/i) != -1 ? 'string' : code;
    ...
    if (profile || (!NATIVE_TYPES.test(code) && schema)){
      code = { type: schema && schema.id ? schema.id.type : 'string', ref: profile ? profileType : code };
    }    
  ...
  /**
   * sortProfileTypes Sorts profiles-types so Extension model goes first and profiles-resources so Resource goes first
   * @return {undefined}
   */
  sortProfileTypes(json, key) {

    let { entry } = json;

    //find first resource.id == 'Extension'
    const idx = entry.findIndex(x => x.resource.id == key);
    ...
    entry.splice(idx, 1);
    entry.splice(toIdx, 0, element);

    return json;
  }

  handleMongooseReserved(key) {
    let _key = key;
    const i = key.indexOf('.');
    if (i != -1){
      _key = key.substring(0, i);
    }

    if (Object.keys(mongoose.Schema.reserved).indexOf(_key) != -1) {
      return `_${key}`;
    }
  ...
  processProfiles() {
    ...
    for (let file of [profilesTypes, profilesResources, profilesOthers]) {
      ...
      //Sorts profiles-types so Extension model goes first and profiles-resources so Resource goes first
      json = this.sortProfileTypes(json, 'Extension');
      json = this.sortProfileTypes(json, 'Resource');
        ...
        if (id && !NATIVE_TYPES.test(id) && snapshot && snapshot.element) {
          ...
          const func = (m, d) => {
            ...
            let pt = d.path.replace(rgx, '')
              .replace(/\[|\]/g,'_'); // ie author[x] in Annotation
            
            //handle keys with mongoose reserved words
            pt = this.handleMongooseReserved(pt);

            //1. the root model won't have a short attribute, **I think** 
            //2. path should not equal model
            if (id.toLowerCase() != pt.toLowerCase() && short && type){
